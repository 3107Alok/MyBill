...
class BillingBloc extends Bloc<BillingEvent, BillingState> {
  ...
  Future<void> _onSaveBill(
    ...
    emit(state.copyWith(isSavingBill: true, saveBillSuccess: false));

    try {
      final billItems = state.cartItems
      ...
      final box = HiveDatabase.billBox;
      await box.put(billModel.id, billModel);

      emit(state.copyWith(
        isSavingBill: false,
        saveBillSuccess: true,
        cartItems: const [], // Clear the cart after saving successfully
      ));
      
      // Auto-load updated history
      add(LoadBillHistoryEvent());
    } catch (e) {
      emit(state.copyWith(
          isSavingBill: false, error: 'Failed to save bill: $e'));
    }
  }

  Future<void> _onLoadBillHistory(
      LoadBillHistoryEvent event, Emitter<BillingState> emit) async {
    try {
      final box = HiveDatabase.billBox;
      ...
      emit(state.copyWith(
        billHistory: bills.map((b) => b.toEntity()).toList(),
      ));
    } catch (e) {
      emit(state.copyWith(error: 'Failed to load billing history: $e'));
    }
  }

  Future<void> _onDeleteBill(
      DeleteBillEvent event, Emitter<BillingState> emit) async {
    try {
      final box = HiveDatabase.billBox;
      await box.delete(event.billId);
      add(LoadBillHistoryEvent());
    } catch (e) {
      emit(state.copyWith(error: 'Failed to delete bill: $e'));
    }
  }
}
