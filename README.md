# 🛒 MyBill - Premium Glassmorphic POS & Billing App

**MyBill** is a feature-rich, high-performance offline-first billing and Point of Sale (POS) application built with Flutter. It is designed with a premium **Glassmorphic UI** to deliver a stunning visual experience while providing robust retail checkout operations, automated barcode scanning, receipt PDF generation/sharing, sales statistics tracking, and thermal Bluetooth printing.

## 🎯 Project Scope

This application serves as a complete offline-first POS system for small to medium-sized retail shops. It streamlines the checkout process, catalog management, receipt generation, and sales history logging securely entirely on-device.
...
- **Sales History & Stats Dashboard**: Stores finalized order receipts in a local database with interactive filters, detail drilldown, reprint options, and sales statistics (Today's vs 30-Day Revenue).
- **PDF Document Generation & Sharing**: Native on-device PDF receipt rendering utilizing `pdf` and `printing` for instant sharing via WhatsApp, Email, or printing on standard office/POS printers.
- **Glassmorphic Aesthetic UI**: Premium design overhaul incorporating backdrop blur overlays, glowing border details, and a dark slate palette.
